...
async def get_all_prices(ticker: str = Query(...)):
    ...
    :param ticker: Название валюты (тикер).
    :return: Список словарей с данными о цене валюты с указанием даты.
    """
    
        
    try:
        logging.info("connection_db ")
        db = create_connection_db() # подключение к базе данных
        cur = db.cursor()
        prices = cur.execute("SELECT * FROM prices WHERE ticker_usd == ?", (ticker,)).fetchall()
        db.close()
        if prices:
...
async def get_latest_price(ticker: str = Query(...)):
    ...
    :param ticker: Название валюты (тикер).
    :return: Словарь с данными о последней цене валюты.
    """
...
async def get_price_by_date(ticker: str = Query(...), start_date: int = Query(...), end_date: int = Query(...)):
    ...
    :param ticker: Название валюты (тикер).
    :param start_date: Начальная дата фильтра в формате Unix.
    :param end_date: Конечная дата фильтра в формате Unix.
    :return: Список словарей с данными о цене валюты с указанием даты.
    """
